...
{
    function questions_and_paper()
    {
        $this->ki_theme->set_title('Questions And Paper', true);
        $this->view("questions_and_paper");
    }
    ...
    function request()
    {
        if(isset($_GET['id']) && intval($_GET['id']) > 0){
            $exam = $this->center_model->get_requests($_GET['id']);
            if($exam->num_rows()){
                $this->set_data('exam',$exam->row_array());
            } 
        }

        if(isset($_GET['type'])){
            $this->set_data('type',$_GET['type']);
        } else {
            $this->set_data('type','');
        }
