...
class Document extends MY_Controller
{
    ...
    function admit_card()
    {
        ...
        if ($get->num_rows()) {
            ...
            $getCentre = $this->center_model->get_center($get->row('institute_id'));
            $centreRow = $getCentre->row();
            $this->set_data('c_signature', $centreRow->signature);
            $this->ki_theme->generate_qr($get->row('admit_card_id'), 'admit_card', current_url());
            $this->set_data('date', date('d-m-Y', strtotime($get->row('exam_date'))));
            $this->set_data('time', date('h:i A', strtotime($get->row('exam_date'))));
            $pdfContent = $this->parse('admit-card',$get->row_array());
    ...
    function marksheet()
    {
        ...
        if ($get->num_rows()) {
            ...
            $getCentre = $this->center_model->get_center($center_id);
            $centreRow = $getCentre->row();
            $this->set_data('c_signature', $centreRow->signature);
            ...
            if (PATH == 'isdmedu') {
                ...
                $this->set_data('from_date', $row->batch_from_date);
                ...
                $toDateString = strtotime('-1 month', $toDateString);
                $this->set_data('to_date', $row->batch_to_date);
            }
    ...
    function certificate()
    {
        ...
        if ($get->num_rows()) {
            ...
            $getCentre = $this->center_model->get_center($certificate['institute_id']);
            $centreRow = $getCentre->row();
            $this->set_data('c_signature', $centreRow->signature);
            //pre($certificate,true);
            $admissionTime = strtotime($certificate['admission_date']);
            $this->set_data('from_date', $certificate['batch_from_date']);
            ...
            $toDateString = strtotime('-1 month', $toDateString);
            $this->set_data('to_date', $certificate['batch_to_date']);
        ...
        if ($get->num_rows()) {
            ...
            if ($data['status'] && $data['isPending'] == 0 && $data['isDeleted'] == 0) {
                if ($data['valid_upto'] && $data['certificate_issue_date']) {
                    ...
                    if(in_array(PATH,['techno','haptronworld','sewaedu','isdmedu'])){
                        $this->mypdf->addPage('L');
                    }
                    $this->pdf($output);
                } else
