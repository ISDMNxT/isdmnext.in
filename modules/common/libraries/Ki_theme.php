...
class Ki_theme
{
    ...
    function grade($score)
    {
        ...
        if (PATH == 'isdmedu') {*/
            if (intval($score) >= 90 && intval($score) <= 100)
                return 'E';
            else if (intval($score) <= 89 && intval($score) >= 70)
                return " A+&nbsp;";
            else if (intval($score) <= 69 && intval($score) >= 50)
                return 'A';
            else if (intval($score) <= 49 && intval($score) >= 30)
                return 'B';
            else
                return 'C';
        /*}
    ...
    function get_manual_notification($where,$seen = 0){
        if(table_exists('manual_notifications'))
            return $this->CI->db->where('seen',$seen)->where($where)->get('manual_notifications')->result_array();
        return [];
    }
