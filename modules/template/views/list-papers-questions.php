            ...
            <table class="w-100 table table-striped table-bordered border-warning " data-question="">
                <tbody>
                    ...
                    if (count($data)) {
                        ...
                        foreach ($data as $ans) {
                            echo '<tr><td>';
                            echo isset($ans['first']) ? '<span class="fs-4 fw-bold">' . $this->ki_theme->set_class('answers')->set_attribute('data-nquesmark',$ques['negative_marks'])->set_attribute('data-quesmark',$ques['marks'])->set_attribute('data-ques',$ques_id)->set_attribute('id', 'ques_' . $i++ . '_' . $ques_id)->html($ans['first'])->radio('answer_' . $ques_id, @$ansers_id[$ans_key], '', 'text-dark') . ' </span>' : '';
                            echo '</td><td>';
                            echo isset($ans['second']) ? '<span class="fs-4 fw-bold">' . $this->ki_theme->set_class('answers')->set_attribute('data-nquesmark',$ques['negative_marks'])->set_attribute('data-quesmark',$ques['marks'])->set_attribute('data-ques',$ques_id)->set_attribute('id', 'ques_' . $i++ . '_' . $ques_id)->html($ans['second'])->radio('answer_' . $ques_id, @$ansers_id[++$ans_key], '', 'text-dark') . '</span>' : '';
                            echo '</></tr>';
                            $ans_key++;
                        }
                    } else {
        ...
        <div class="{card_class} card-flush" id="exam-instru">
            <div class="card-body">
                ...
                <div class="d-flex fw-semibold align-items-center">
                    ...
                    <div class="fw-bolder text-gray-700 text-xxl-end ttl-max-questions"><?= count($questions) ?></div>
                </div>
            </div>
            <div class="card-body">
...
<script type="text/javascript">
    $(document).ready(function () {
        // `remainingTime` की ID वाले एलीमेंट से समय प्राप्त करना
        let remainingTime = parseInt($('#remainingTime').text().match(/\d+/)[0], 10) * 60; // मिनट को सेकंड में कन्वर्ट करना (e.g., "30 Minuts" to 1800 seconds)
        ...
        const countdownInterval = setInterval(() => {
            ...
            // मिनट और सेकंड को फॉर्मेट करना
            const minutes = Math.floor(remainingTime / 60);
            const seconds = remainingTime % 60;

            // `remainingTime` वाले एलीमेंट में अपडेट करना
            $('#remainingTime').text(`Remaining Time: ${minutes} Minutes ${seconds} Seconds`);

            // जब टाइम खत्म हो जाए
            if (remainingTime <= 0) {
                clearInterval(countdownInterval); // टाइमर बंद कर देना
                Swal.fire({
                    icon: 'warning',
                    title: 'Time Over',
                    text: 'Your time is up! Submitting the exam...',
        ...
        }, 1000); // हर एक सेकंड पर अपडेट करना
    });
</script>