document.addEventListener('DOMContentLoaded', function (e) {
    ...
    if (form) {
                ...
                if (res.status) {
                    ...
                    if (res.courses.length) {
                        ...
                        $.each(res.courses, function (index, course) {
                            ...
                            if(tcourse.val() == course.course_id){
                                sel = 'selected="selected"';
                            }
                            options += `<option data-price_show="${show}" ${sel} value="${course.course_id}" data-course_fee="${course.course_fee}" data-course_royality_fees="${course.royality_fees}" data-kt-rich-content-subcontent="${course.duration} ${course.duration_type}">${course.course_name}</option>`;
                        });
                    }
                    ...
                    if(parseInt(tcourse.val()) > 0){
                        course_box.trigger("change");
                    }

                    if(parseInt(tbatch.val()) > 0){
                    ...
                    $.AryaAjax({
                        ...
                        if (res.status) {
                            $.each(res.batches, function (index, batche) {
                                ...
                                if(tbatch.val() == batche.id){
                                    sell = 'selected="selected"';
                                }
                                options += `<option value="${batche.id}" ${sell} data-kt-rich-content-subcontent="${batche.duration}">${batche.batch_name}</option>`;
                            });
                        }
                        else {
        ...
        if (login_type == 'center' || parseInt($('#tbatch').val()) > 0) {
