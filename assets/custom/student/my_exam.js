document.addEventListener('DOMContentLoaded', async function () {
    ...
    $(document).on('click', '.ok-btn', function () {
        ...
        $.AryaAjax({
            ...
            myModel(e.title, `${e.content} <input type="hidden" class="isnegativemarking" value="${e.negativemarking}"> <input type="hidden" class="student_exam_id" value="${id}">`).then((d) => {
    ...
    function submitExam(form) {
        ...
        let rightQuestionsMarks = 0;
        let ttlMarks = 0;
        let exam_id = $(form).find('.exam_id').val();
        let submitList = new Array();
        let student_exam_id = $(form).find('.student_exam_id').val();
        let isnegativemarking = $(form).find('.isnegativemarking').val();
        answers.each(function () {
            let q_id = $(this).data('ques');
            let q_marks = $(this).data('quesmark');
            let q_negativemarks = $(this).data('nquesmark');
            let ans_id = $(this).val();
            console.log();
            if (allQuestions.hasOwnProperty(q_id)) {
                if (ans_id == allQuestions[q_id]){
                    rightQuestions++;
                    rightQuestionsMarks = parseInt(rightQuestionsMarks)+parseInt(q_marks);
                } else {
                    if(isnegativemarking == 'Y'){
                        rightQuestionsMarks = parseInt(rightQuestionsMarks)-parseInt(q_negativemarks);
                    }
                }
            }
            ttlMarks = parseInt(ttlMarks)+parseInt(q_marks);
            submitList.push({
                'question_id': q_id,
                'answer_id': ans_id,
        ...
        let percentage = (rightQuestionsMarks / ttlMarks) * 100;
        ...
        $.AryaAjax({
            ...
            data: {
                ...
                ttl_right_answers_marks: rightQuestionsMarks,
