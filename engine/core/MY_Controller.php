...
class MY_Controller extends MX_Controller
{
    ...
    function __construct()
    {
        ...
        $this->public_data = [
            ...
            'sr_button' => $this->ki_theme->publish_button("Send Request"),
            'send_notification' => $this->ki_theme->set_class('send')->save_button("Send Notification", 'send'),
            ...
            'appr_button' => $this->ki_theme->set_class('save-btn')->save_button('Approve', 'save-2'),
            'rejt_button' => $this->ki_theme->set_class('delete-btn')->save_button('Reject', 'save-2'),
        ...
        $getsignature = $this->center_model->get_admin();
        if ($getsignature->num_rows()) {
            $this->set_data('admin_signature', $getsignature->row("value"));
        }
        ...
        if ($this->center_model->isAdminOrCenter()) {
            ...
            if($this->session->userdata('admin_type') == 'center' && !empty($this->session->userdata('staff_id'))){
                ...
                $getStaff = $this->center_model->get_center($this->session->userdata('staff_id'), 'staff');
                $staffRow = $getStaff->row();
                $this->set_data('profile_image', (file_exists('upload/' . $staffRow->image) ? base_url('upload/' . $staffRow->image) : base_url('assets/media/avatars/300-3.jpg')));
                $this->set_data('center_signature', (file_exists('upload/' . $centreRow->signature) ? base_url('upload/' . $centreRow->signature) : base_url('assets/media/auth/welcome.png')));
                $this->set_data('center_logo', (file_exists('upload/' . $centreRow->logo) ? base_url('upload/' . $centreRow->logo) : base_url('assets/media/auth/welcome.png')));
                $this->set_data([
                    'owner_name' => $staffRow->name,
                    'owner_email' => $staffRow->email,
                    'owner_phone' => $staffRow->contact_number,
                ...
                $this->set_data('center_signature', (file_exists('upload/' . $centreRow->signature) ? base_url('upload/' . $centreRow->signature) : base_url('assets/media/auth/welcome.png')));
                $this->set_data('center_logo', (file_exists('upload/' . $centreRow->logo) ? base_url('upload/' . $centreRow->logo) : base_url('assets/media/auth/welcome.png')));
                $typek = $this->center_model->login_type();
                if($this->center_model->login_type() == 'master_franchise'){
                    $typek = 'master franchise';
                }
                $this->set_data([
        ...
        if ($this->center_model->isEmployer()) {
            ...
            $this->set_data('profile_image', (file_exists('upload/' . $centreRow->image) ? base_url('upload/' . $centreRow->image) : base_url('assets/media/avatars/300-3.jpg')));
            $this->set_data('employer_signature', (file_exists('upload/' . $centreRow->signature) ? base_url('upload/' . $centreRow->signature) : base_url('assets/media/auth/welcome.png')));
            $this->set_data('employer_logo', (file_exists('upload/' . $centreRow->logo) ? base_url('upload/' . $centreRow->logo) : base_url('assets/media/auth/welcome.png')));
            $typek = $this->center_model->login_type();
            $this->set_data([
                'owner_name' => $centreRow->name,
                'owner_email' => $centreRow->email,
    ...
    function view($view, $data = [], $return = false)
    {
        ...
        if ($this->session->has_userdata('admin_login') == true) {
            ...
            if ($this->center_model->isEmployer()){
                return $this->parse('employer/main', $this->public_data, $return);
            } else {
                return $this->parse('admin/main', $this->public_data, $return);
            }
        } else {
            $ruri = explode("/",$_SERVER['REQUEST_URI']);
            if(!empty($ruri[2]) == 'employer' && !empty($ruri[3]) == 'index'){
                $this->parser->parse('login/employer_login', $this->public_data);
            } else {
