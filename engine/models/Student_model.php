...
class Student_model extends MY_Model
{
    function get_switch($case = 'all', $condition = [], $withCenter = true)
    {
        ...
        $this->db->select('        
                b.batch_name,
                DATE_FORMAT(b.from_date,"%d-%m-%Y") as batch_from_date,
                DATE_FORMAT(b.to_date,"%d-%m-%Y") as batch_to_date,
        ...
        switch ($case) {
            ...
            case 'assign_exam_student_list':
                ...
                break;
            case 'student_exams':
                $this->db->select('es.id as assign_exam_id,es.*,e.*, ess.subject_name,qp.*,es.status as paper_status');
                $this->db->join('exams_master_trans as es', 'es.student_id = s.id ');
                $this->db->join('subject_master as ess', 'ess.id = es.subject_id ');
                $this->db->join('exams_master as e', "e.id = es.exam_master_id and e.status = 1 and e.is_admin_approved = 2 and e.isDeleted = 0");
                $this->db->join('question_paper as qp', "qp.id = es.question_paper_id");
                // $this->db->group_by('s.id');
            ...
            case 'all':
                ...
                break;
            case 'all_student':
                $this->db->where('s.admission_status', $condition['admission_status']);
                $this->db->where_in('s.center_id', $condition['all_c']);
                break;
            ...
            case 'fetch_fee_transactions':
                $this->db->select('sf.id,sf.fees_type,sf.fees_period,sf.payment_type,sf.total_amount,sf.paid_amount,sf.pending_amount,sft.id as trans_id,sft.payment_id,sft.payment_date,sft.amount,sft.description')->join('student_fees as sf', "sf.student_id = s.id")->join('student_fees_trans as sft', "sf.id = sft.student_fees_id");
                $this->myWhere('sft', $condition);
                break;
            case 'fetch_fee_transactions_all':
            
                $this->db->select('sf.id,sf.fees_type,sf.fees_period,sf.payment_type,sf.total_amount,sf.paid_amount,sf.pending_amount')->join('student_fees as sf', "sf.student_id = s.id");
                $this->myWhere('sf', $condition);
                break;
            case 'fetch_fee_transactions_group_by':
                $this->db->select('sft.*,SUM(sft.total_amount) as ttl_amount,SUM(sft.pending_amount) as ttl_pending,SUM(sft.paid_amount) as ttl_paid')
                    ->join('student_fee_transactions as sft', "sft.student_id = s.id");
                //$this->db->group_by('sft.payment_id');
                $this->myWhere('sft', $condition);
                break;
            case 'fetch_fee_transactions_ttl':
                $this->db->select('sft.*,SUM(sft.total_amount) as ttl_fee,SUM(sft.paid_amount) as ttl_paid,SUM(sft.pending_amount) as ttl_pending')->join('student_fees as sft', "sft.student_id = s.id");
    ...
    function get_all_enquiry($id = 0)
    {
        ...
        $this->db->where('s.enquiry_status', 0);
        if(intval($id) > 0){
            $this->db->where('s.id', $id);
        }
    ...
    function get_switch_old($case = 'all', $condition = [], $withCenter = true)
    {
        extract($condition);
        $this->db->select('        
                b.batch_name,
        ...
        switch ($case) {
            ...
            case 'fetch_fee_transactions':
                ...
                break;
            case 'fetch_fee_transactions_group_by':
                $this->db->select('sft.*,SUM(sft.payable_amount) as ttl_amount,SUM(sft.discount) as ttl_discount,SUM(sft.paid_amount) as ttl_paid')
                    ->join('student_fee_transactions as sft', "sft.student_id = s.id");
                $this->db->group_by('sft.payment_id');
                $this->myWhere('sft', $condition);
                break;
            case 'fetch_fee_transactions_ttl':
                $this->db->select('sft.*,SUM(sft.payable_amount) as ttl_fee,SUM(sft.discount) as ttl_discount,SUM(sft.paid_amount) as ttl_paid')->join('student_fee_transactions as sft', "sft.student_id = s.id");
                $this->db->group_by('sft.student_id', $student_id)->limit(1);
                $this->myWhere('sft', $condition);
                break;
            case 'get_admit_card':
    ...
    function get_student_via_center_course_batch($condition = [])
    {
        extract($condition);
        $this->db->select('        
                b.batch_name,
    ...
    function fetch_fee_transactions_all($where)
    {
        return $this->get_switch('fetch_fee_transactions_all', $where);
    }
    ...
    function get_fee_transcations_ttl($where)
    {
        ...
        $data = ['ttl_fee' => 0, 'ttl_pending' => 0, 'ttl_paid' => 0];
        if ($ttlrow->num_rows()) {
            $data = [
                'ttl_fee' => $ttlrow->row('ttl_fee'),
                'ttl_pending' => $ttlrow->row('ttl_pending'),
                'ttl_paid' => $ttlrow->row('ttl_paid'),
            ];
        }
        return $data;
    }
    ...
    function get_all_student($where = [])
    {
        if ($this->center_model->isMaster()) {
            return $this->get_switch('all_student', [
    ...
    function course_subject_new($where)
    {
        return $this->db->select('a.*,(SELECT study_material from isdm_subject_master WHERE id = a.subject_id) AS study_material')->where($where)
            ->order_by('a.seq', 'ASC')
            ->get('subjects a');
    }
    ...
    {
        return $this->db->where('isDeleted', 0)->where('status', 1)
            ->get('course');
    }
    ...
    function system_master_subjects($deleted = 0)
    {
        $this->db->select('s.*')
            ->from('subject_master as s')
            ->where('s.isDeleted', $deleted);
        return $this->db->get();
    }

    function marksheet_request($center_id = 0)
    {
        $this->db->select('
            m.id,
            c.institute_name as center_name,
            s.name as student_name,
            s.roll_no,
            UPPER(co.course_name) as course_name,
            em.exam_title,
            m.date,
            m.status,
            DATE_FORMAT(b.from_date, "%d-%m-%Y") as batch_from_date,
            DATE_FORMAT(b.to_date, "%d-%m-%Y") as batch_to_date
        ')
        ->from('marksheets_request as m')
        ->join('centers as c', 'c.id = m.center_id', 'left')
        ->join('course as co', 'co.id = m.course_id', 'left')
        ->join('students as s', 's.id = m.student_id', 'left')
        ->join('batch as b', 'b.id = s.batch_id', 'left')
        ->join('exams_master as em', 'em.id = m.exam_id', 'left');
    
        if ($this->isCenter()) {
            $this->db->where('m.center_id', $this->loginId());
        }
        ...
        // ✅ Only sort by m.id DESC to show latest first
    ...
    function feestatus($status)
    {
        $ss = "";
        if($status == 'pending'){
            $ss = ",SUM(sf.pending_amount) as fees";
        }

        if($status == 'collection'){
            $ss = ",SUM(sf.paid_amount) as fees";
        }
        $this->db->select('s.roll_no,s.name as student_name,UPPER(co.course_name) as course_name, co.duration,
                co.duration_type'.$ss)
            ->from('student_fees as sf')
            ->join('students as s', 's.id = sf.student_id', 'left')
            ->join('centers as c', 'c.id = sf.center_id', 'left')
            ->join('course as co', 'co.id = sf.course_id', 'left');
            
            
        if ($this->isCenter()){
            $this->db->where('sf.center_id', $this->loginId());
        }

        $this->db->group_by('sf.student_id');
